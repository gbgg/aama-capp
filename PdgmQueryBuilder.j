...
{
  // CPLog("%@ outlineView: child:%@ ofItem:", self, index);
...
{
  // CPLog("%@ 2 outlineView: isItemExpandable:%@", self);
  ...
  if (ftype == COLLECTION & [[item objectForKey:@"CHILDREN"] count] > 0) {
    // CPLog(@"YES");
    return YES;
  } else {
    // CPLog(@"NO");
...
{
  // CPLog("%@ outlineView:%@ numberOfChildrenOfItem:%@", self); //, outlineView, item);
...
{
  // CPLog("%@ outlineView: objectValueForTableColumn:%@ byItem:%@", self); //, tableColumn, item);
  // CPLog(@"item keys: " + [item allKeys]);
  //  CPLog(@"item vals: " + [item allValues]);
  // if ( [item objectForKey:@"Type"] == COLLECTION ) {
  //   return;
  // }
